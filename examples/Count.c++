...
std::ptrdiff_t count_if_1 (const char* b, const char* e, bool (*f) (char)) {
    std::ptrdiff_t c = 0;
    while (b != e) {
        if (f(*b))
            ++c;
...
std::ptrdiff_t count_if_2 (II b, II e, UP f) {
    std::ptrdiff_t c = 0;
    while (b != e) {
        if (f(*b))
            ++c;
...
int main () {
    ...
    assert(count_if_1(a, a + s, my_isupper) == 2);
    assert(count_if_1(a, a + s, p)          == 2);

    assert(count_2   (a, a + s, 'b')        == 2);
    assert(count_2   (a, a + s, 'd')        == 0);
    assert(count_if_2(a, a + s, my_isupper) == 2);
    assert(count_if_2(a, a + s, p)          == 2);
